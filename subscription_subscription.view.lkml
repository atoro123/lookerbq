view: subscription_subscription {
  ...
  dimension: clean_cancel {
    ...
    when substr(${cancel_reason},0,2) = '57' then 'Subscription Import'
    when substr(${cancel_reason},0,2) = '58' then 'Subscription Duration Ended'
    when substr(${cancel_reason},0,2) = '59' then 'I only need diapers'
    when substr(${cancel_reason},0,2) = '60' then 'I only need wipes'
    when substr(${cancel_reason},0,2) = '61' then 'Better price on the Holmes.com site'
    when substr(${cancel_reason},0,2) = '62' then 'GDPR Account Deletion Request'
    when substr(${cancel_reason},0,2) = '63' then 'CCPA Account Deletion Requestion'
    when substr(${cancel_reason},0,2) = '80' then 'The new + improved diapers dont work for me'
