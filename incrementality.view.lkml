view: incrementality {
  derived_table: {
    ...
    sql: SELECT *,
      ...
      SUM(IF(DATE(logged) BETWEEN DATE_SUB(A.mid_date, INTERVAL 90 DAY) AND A.mid_date, B.quantity, 0)) purchases_pre,
      SUM(IF(DATE(logged) BETWEEN DATE_SUB(A.mid_date, INTERVAL 90 DAY) AND A.mid_date, 1, 0)) transactions_pre,
      SUM(IF(DATE(logged) BETWEEN DATE_SUB(A.mid_date, INTERVAL 90 DAY) AND A.mid_date, B.total, 0)) spend_pre,
      SUM(IF(DATE(logged) BETWEEN A.mid_date AND DATE_ADD(A.mid_date, INTERVAL 90 DAY), B.quantity, 0)) purchases_post,
      SUM(IF(DATE(logged) BETWEEN A.mid_date AND DATE_ADD(A.mid_date, INTERVAL 90 DAY), 1, 0)) transactions_post,
