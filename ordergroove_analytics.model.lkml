...
explore: event_log {
  ...
  explore: send_now_event_log {
    view_label: "VSI Fraud"
    hidden: yes
    join: addres_change_event_log {
      sql_on: ${addres_change_event_log.Merchant_User_ID} = ${send_now_event_log.Merchant_User_ID} and ${addres_change_event_log.Customer_ID} = ${send_now_event_log.Customer_ID}
      where ${addres_change_event_log.Logged_Date_date} - ${send_now_event_log.Logged_Date_date} = 0;;
      relationship: one_to_one
    }
    join: order_order {
      sql_on: ${order_order.id} = ${send_now_event_log.Order} ;;
      relationship: one_to_one
    }
    join: order_item {
      sql_on: ${order_item.order_id} = ${order_order.id} ;;
